...
pylab.show()
#Um gráfico popular para verificar a distribuição de uma amostra de dados é o gráfico quantil-quantil, gráfico Q-Q ou gráfico QQ. 
#Uma combinação perfeita para a distribuição será mostrada por uma linha de pontos em um ângulo de 45 graus da parte inferior esquerda do gráfico para o canto superior direito. 
#Muitas vezes, uma linha é desenhada no enredo para ajudar a tornar essa expectativa clara. Desvios pelos pontos da linha mostram um desvio da distribuição esperada.
...
#Intanciando as variaveis para analise de residuos
model = Ridge()
...
#DEFINIÇÃO DOS DADOS DE TREINO E TESTE SEM TRATAMENTO #NORMAL Q-Q RESIDUAL PLOT e Residual Plot DADOS TRATADOS IQR
...
#### DATA BASE REMOVIDO OS OUTLIERS
"""
...
print("Linear Regression RMSE SEM OUTLIERS erro de: +/-", lin_rmse)
...
multi_model = ols('price ~ year + mileage + vol_engine + Mark +	Model	+ Fuel', data=DTCdf).fit()
...
lin_rmse = np.sqrt(mse)
print("Linear Regression RMSE COM OUTLIERS erro de: +/-", lin_rmse)
...
lin_rmse = np.sqrt(mse)
print("Random Forest Regressor RMSE SEM OUTLIERS erro de: +/-", lin_rmse)
...
# calculando o root mean squared error
mse = mean_squared_error(y_test2, y_predRFR2)
lin_rmse = np.sqrt(mse)
print("Random Forest Regressor RMSE SEM OUTLIERS erro de: +/-", lin_rmse)
